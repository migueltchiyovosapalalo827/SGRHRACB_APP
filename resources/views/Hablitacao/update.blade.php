...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('hablitacao.update', ['hablitacao' => $hablitacao->id]) }}" method="post"
                        ...
                        <select class="form-control select2 @error('efectivo') is-invalid @enderror" style="width: 100%;"
                            ...
                            @foreach ($efectivos as $efectivo)
                                <option value="{{ $efectivo->id }}"
                                    {{ $efectivo->id == $hablitacao->efectivo->id ? 'selected' : '' }}>
                    ...
                    <div class="col-sm-8">
                        <div class="input-group">
                            ...
                            <input type="text" name="nome" class="form-control @error('nome') is-invalid @enderror"
                                value="{{ $hablitacao->nome ?? old('nome') }}" placeholder="nome" autocomplete="off">
                            @error('nome')
                                <div class="invalid-feedback">
                                    <h6>{{ $message }}</h6>
                                </div>
                    ...
                    <div class="col-sm-8">
                        <select id="inputStatus" class="form-control custom-select @error('tipo') is-invalid @enderror"
                            ...
                            @foreach ($tipos as $tipo)
                                <option value="{{ $tipo->value }}" {{ old('tipo') ?? $hablitacao->tipo == $tipo->value ? 'selected' : '' }}>
                                    {{ $tipo->value }}</option>
                            @endforeach
                        </select>
                        @error('tipo')
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-8">
                        <input type="text" id="inputClientCompany"
                            class="form-control @error('nivel') is-invalid @enderror" name="nivel" placeholder="Nivel" value="{{$hablitacao->nivel ?? old('nivel')}}">
                        @error('nivel')
                            <div class="invalid-feedback">
                                <h6>{{ $message }}</h6>
                            </div>
