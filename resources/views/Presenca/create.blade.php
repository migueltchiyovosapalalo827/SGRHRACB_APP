...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="<?= route('presenca.store') ?>" method="post" class="form-horizontal">
                @csrf
                    ...
                    <div class="col-sm-8">
                        <div class="input-group">
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="ausente" value="1">
                                <label class="form-check-label">Sim</label>
                              </div>
                              <div class="form-check">
                                <input class="form-check-input" type="radio" name="ausente" checked value="0">
                        ...
                        <select id="inputStatus" class="form-control custom-select @error('motivo') is-invalid @enderror"
                            ...
                            @foreach ($motivos as $motivo)
                                <option value="{{ $motivo->value }}"
                                    {{ old('motivo') ?? 'NENHUM' == $motivo->value ? 'selected' : '' }}>
                                    {{ $motivo->value }}</option>
                            @endforeach
                        </select>
                        @error('motivo')
